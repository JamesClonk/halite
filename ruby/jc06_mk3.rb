...
def productive_target(site, loc)
  target = GameMap::CARDINALS.map { |dir| {:direction => dir, :site => map.site(loc, dir), :location => map.find_location(loc, dir)} }
...
def evaluate_production(cell)
  ...
  GameMap::CARDINALS.each do |direction|
...
def at_border?(loc)
  GameMap::CARDINALS.any? {|d| map.site(loc, d).owner != tag}
end
